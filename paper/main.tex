...
robustness at zero clean-accuracy cost, and the decomposition attributes the
...
subtracts 9.5\,pp of BSM packet-reception ratio at 100\,m legit-link
...
\begin{enumerate}
  ...
  evaluation attack ($+6.7$\,dB against the converged adaptive attack;
...
model/protocol; model-seed spread is reported separately). The honesty box records the prototype scale; the
...
Adaptive attack & PGD-50, 5 restarts (zero-init + 4 random), best-of-R per sample \\
...
Confidence & Wilson 95\% binomial CIs on every ASR cell (results/w12\_confidence\_intervals.json) \\
\bottomrule
\end{tabular}
\end{table}
...
($8.5$\,dB of the $10.4$\,dB total shift); the four random restarts add
...
column is the canonical protocol from Table~\ref{tab:protocol}; the
$\Delta$ column is the defense margin over the undefended control under
the SAME attack protocol. Wilson 95\% CIs on the MEAPs are
$\pm$1.3--1.6\,dB.}
\label{tab:adaptive}
\begin{tabular}{lccc|cc}
\toprule
 & \multicolumn{3}{c|}{mask MEAP (dB)} & \multicolumn{2}{c}{defense margin (dB)} \\
Victim & PGD-10 & PGD-50 R$=$1 & PGD-50 R$=$5 & PGD-10 & adaptive \\
\midrule
Undefended & $-37.1$ & --- & $-39.4$ & --- & --- \\
AT & $-23.1$ & $-31.6$ & $-32.7$ & $+14.0$ & $+6.7$ \\
TRADES & $-19.1$ & --- & $-29.6$ & $+18.0$ & $+9.9$ \\
\bottomrule
\end{tabular}
\end{table}
...
conditional ASR, eligibility, and the win histogram.
...
PSD that steps down to $-3$, $-9$, $-17$, $-27$\,dBm/MHz at $\pm$4.5,
$\pm$5, $\pm$10, $\pm$15\,MHz offsets from the attacker's carrier,
...
timing), the legitimate BSM PRR at 100\,m drops from 0.857 to 0.762
($-9.5$\,pp; $\sim$95 extra lost BSMs per 1000 transmitted); at 150\,m,
from 0.377 to 0.335; at the $-10$\,dB operating point, 0.857$\to$0.676
...
defense study and the TRADES ablation remain single-seed (seed 42), as does
the adaptive-attack evaluation (attack seed 7, 5 restarts). Confidence
intervals are Wilson binomial on sampling noise only; model-seed variance
(1.1\,dB) is a separate, additive source.
...
false-IDLE point costs 9.5\,pp of BSM PRR at 100\,m. Two protocol
...
$+9.9$\,dB with a TRADES variant at 0.75\,pp clean cost. The
