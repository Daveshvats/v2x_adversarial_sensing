...
\author[inst1]{Davesh Vats\corref{cor1}}
...
carried over from our prior work~\cite{vats2026legacy} for architectural
...
\mathrm{MEAP}_{\mathrm{genie}}$: the extra power a rule-abiding attacker must
spend to achieve the same attack success. PoC is
...
carry paired-bootstrap 95\% CIs as well
...
``phase-aware'' framing of our prior work~\cite{vats2026legacy}---an honest
...
V1.2.1, Table~7 (10\,MHz channel). The template is a flat
$23$\,dBm/MHz in-channel PSD held flat to $\pm$4.5$\,$MHz that steps down
to $-3$, $-9$, $-17$, $-27$\,dBm/MHz at $\pm$5, $\pm$5.5, $\pm$10,
$\pm$15\,MHz offsets from the attacker's carrier, interpolated linearly
in dB. Values were extracted from the official ETSI PDF and committed as
machine-readable tables with provenance and file hash
...
domain}: for every overlapping 1-MHz measurement band (centers every
0.5\,MHz, linear-in-dB interpolated limits), the mean per-bin FFT power
must stay within the Table-7 value at that center offset; the projection is
...
domain (100-kHz RBW for centers within 1\,MHz of the channel edge,
1-MHz RBW elsewhere, overlapping centers), the compliant MEAP is
...
while the defenses are also evaluated under converged adaptive
...
\bibitem{vats2026legacy} D. Vats, ``Dual-stream phase-aware Inception-Time
CNN for adversarially robust spectrum sensing in V2X networks,''
unpublished manuscript, 2026 (preprint archived in this repository,
\texttt{paper/legacy/}).
